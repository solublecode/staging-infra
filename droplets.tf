resource "digitalocean_droplet" "server" {
    ...
    provisioner "file" {
        ...
        destination = "$HOME/consul/consul-connect-enable.hcl"
    }

    # Nomad files
    provisioner "file" {
        source      = "${path.root}/scripts/nomad/server/server.hcl"
        destination = "$HOME/nomad/server.hcl"
    }

    provisioner "file" {
        source      = "${path.root}/scripts/nomad/client/client.hcl"
        destination = "$HOME/nomad/client.hcl"
    }
    ...
    provisioner "file" {
        ...
        destination = "$HOME/prepare.sh"
    }

    provisioner "file" {
        source      = "${path.root}/scripts/vault/vault-config.hcl"
        destination = "$HOME/vault/vault-config.hcl"
    }
